...
function Users({ token, competitions, units, currentUser }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    const poll = async () => { try { const payload = await api('/manage/users/candidates/import/status', { token }); if (!active || !payload.job || payload.job.id !== bulkResult.jobId) return; setBulkResult({ ...payload.job, jobId: payload.job.id }); if (payload.job.status === 'completed') reload(); } catch { /* L?n h?i ti?p theo s? t? th? l?i. */ } };
...
function SiteEditor({ token, onChanged }) {
  ...
  async function upload(file, kind) {
    if (!file) return;
    await action.run(async () => {
      if (file.size > 5 * 1024 * 1024) throw new Error('Tệp phải có dung lượng tối đa 5 MB.');
  ...
  return <section className="content-section">
    ...
    <ResourceState resource={resource}>
      <form onSubmit={event => {
        event.preventDefault();
        action.run(async () => {
          await api('/manage/site', { token, method: 'PUT', body: data });
          onChanged();
        ...
        <fieldset className="form-grid" disabled={action.busy}>
          ...
          <label>Tệp tin tức / thể lệ (tối đa 5 MB)<input type="file" accept=".pdf,.txt,.docx" onChange={event => upload(event.target.files?.[0], 'news')} /></label>
